...
| 2026-04-26 | build-003 | Advanced PRD build for Spark Galaxy Garden produced a useful 5-task PRD result, then failed to queue canvas with `404 No analysis result` | Spawner PRD bridge endpoints used different state roots: result/write/pending fell back to cwd while load-to-canvas used configured `SPAWNER_STATE_DIR` | Spawner PRD bridge now resolves configured state paths consistently per request, worker prompt receives absolute pending/result paths, targeted bridge tests pass, and the failed Galaxy Garden canvas payload was recovered with 5 nodes / 8 connections / `spark-agi@8789` relay metadata |
| 2026-04-26 | smoke-003-kanban-canvas-retry | `/board` now shows the canvas-created Galaxy Garden mission as completed: `mission-1777196714204 | TAS-5: Manual QA and no-build delivery check` | Canvas emitted `mission-*` ids while Kanban relay previously only accepted `spark-*`; self-originated canvas events were also skipped before board recording | Spawner Mission Control now records local canvas events for Kanban without external relay echo, accepts `mission-*` ids, and the execution panel no longer exposes dispatch/copy plumbing |
| 2026-04-26 | build-005 | Direct build for `./spark-live-direct-probe` queued a valid 2-task canvas, then auto-started two failed missions routed to Z.AI GLM with `fetch failed` | Telegram relay canvas execution hard-forced `zai` instead of the mission default provider, and the auto-run effect did not mark a token in-flight before starting async execution | Spawner execution now resolves relay mission provider through a tested Codex-first helper and guards in-flight auto-run tokens before dispatch; `npm run check` and focused provider tests pass in desktop/live Spawner |
| 2026-04-26 | spark-character-v8 | Style rules for no bold/italic Markdown and shorter scan-friendly paragraphs existed in `persona.v8.md` but were not active in the live module | `persona.v8.md` was untracked in desktop and absent from the live Spark Character module; `persona.latest.txt` still pointed at v7 | Promoted Spark Character persona v8 in desktop/live modules; full desktop/live `python -m pytest` suites pass 55 tests each |
| 2026-04-26 | spark-character-t5-resolution | Cross-provider character eval is the right test for multi-LLM voice drift, but the first shell did not inherit `.env.zai`, so only Codex resolved | The T5 runner depends on provider env being attached to the process; MiniMax remains unconfigured here | Verified `evals/cross_provider.py` resolves `zai` + `codex` after loading `~/.env.zai`; full live command is `python evals/cross_provider.py --providers zai,codex,minimax --judge zai --out evals/_cross_provider_v8_live.json` |
