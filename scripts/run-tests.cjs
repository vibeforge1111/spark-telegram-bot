...
for (const testFile of testsToRun) {
  ...
  const testEnv = {
    ...
    SPARK_NODE_OUTBOUND_AUDIT_PATH: path.join(tempDir, 'node-outbound-audit.jsonl'),
    SPARK_FINAL_ANSWER_GATE_AUDIT_PATH: path.join(tempDir, 'final-answer-gate-audit.jsonl'),
    SPARK_ROUTE_CONFIDENCE_AUDIT_PATH: path.join(tempDir, 'route-confidence-audit.jsonl'),
    SPARK_HARNESS_CORE_LEDGER_PATH: path.join(tempDir, 'harness-core-tool-ledger.jsonl'),
    SPARK_NATURAL_ROUTE_LEDGER_PATH: path.join(tempDir, 'natural-route-ledger.jsonl')
  };
