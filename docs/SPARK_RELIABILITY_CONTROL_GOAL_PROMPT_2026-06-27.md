...
```text
Goal: Continue Spark reliability hardening as an organized control program, not a cleanup sprint. Prove old edges, hidden sources, and live route behavior cannot silently affect current Telegram, Recursive, Spawner, Builder, mission relay, or docs behavior.
...
- Already present: source inventory, render firewall, trace-join checker, live route join proof, proof-capsule coverage, reliability eval coverage, legacy prompt surface gate, capability evidence, surface eval, and full/scoped canary packets.
...
4. Require proof capsules: every action-capable route must emit or join one proof capsule with a strong policy summary; no-action turns must prove no action happened.
