...
7. Show partial trace evidence honestly. The proof panel reports ref-only evidence as joined when a redacted proof ref is present, names the missing capsule separately under `Evidence capsule gaps`, and keeps trace-only rows under `Evidence trace-only` so they are not counted as proof-ref joins.
