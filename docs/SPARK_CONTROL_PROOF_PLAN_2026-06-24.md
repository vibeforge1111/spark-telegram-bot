...
7. Show partial trace evidence honestly. The proof panel reports ref-only evidence as joined when a redacted proof ref is present but the full capsule is missing; trace-only rows stay under `Evidence trace-only` and must not be counted as `Evidence joined`.
