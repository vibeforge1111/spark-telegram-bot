...
Canary packet timestamps are part of the proof contract: `generatedAt` and `evidence.collectedAt` must both be strict millisecond ISO timestamps. Runtime refresh sets `generatedAt` to the collected evidence timestamp, and release checks reject packet metadata that predates the runtime evidence or is more than five minutes future-dated.

The freshness contract now also joins into the embedded command transcripts. `spark os compile --json` must carry a parseable `generated_at`/`generatedAt`, and the fresh-strict control-proof audit transcript must carry its `Generated:` line; both command timestamps must be close to `evidence.collectedAt`. Updating only the packet wrapper timestamp while reusing stale compile or audit output is not release proof.
...
Canary summaries now carry non-blocking release caveats from `spark os compile --json`. Unclassified runtime-ahead-of-registry-pin drift is printed in the human summary and JSON summary as `registry_pin_drift`; intentionally classified local SparkRecursive proof runtimes are printed as `local_runtime_test_artifacts`. Both remain explicit publish/registry handoff items with safe duplicate-truth classification counts, while preserving the Telegram canary release verdict when compile gaps, unhanded dirty runtime state, and privacy checks are clean. Duplicate-truth drift without runtime-ahead or local-runtime-test rows is labeled separately from registry pin drift. Unhanded dirty runtime compile evidence still invalidates the canary packet; dirty owner repos are acceptable only when represented as sanitized `repo_release_blocks` publish handoffs.
...
Live visual canary evidence must use `screenshot:sha256:<digest>` refs in release packets instead of raw `/tmp` screenshot paths. Raw local paths are capture-stage artifacts only; pass them to `--screenshot-file` so the recorder hashes the screenshot and stores only the stable digest ref. Use `--screenshot-ref` only when the digest was already computed outside the recorder.
...
Standard checked-in live canary bundles now keep both human and machine-readable summaries current. `live-canary-summary.md` is the operator-facing report, and `live-canary-summary.json` is generated from the same summary and coverage logic used by strict/release checks. Run-guide record commands include both `--summary-out` and `--summary-json-out`, so a live recapture cannot refresh the markdown summary while leaving the JSON proof packet stale. The selected `outputs/live-canary-safe-first` packet follows this contract too, but remains only a focused selected-case gate until the complete canary pack is run.
...
npm run control:proof:canaries -- --observations outputs/live-canary-observations.json --repair-stale-proof-panels
```
...
`--strict` checks both presence and clean contents for packet evidence: live status, provider status, sync, OS compile, and control-proof audit evidence must include successful command transcripts, not prose summaries. The compile and audit transcripts must carry embedded generation timestamps close to `evidence.collectedAt`, so a fresh wrapper cannot launder stale command output. The control-proof audit must include `--fresh-strict`, `Actionable status: clean`, `Blocking status: clean`, plus structured counters for zero missing evidence, zero missing trace joins, zero missing proof capsules, zero source proof gaps, zero incomplete legacy gap backing, zero raw ref leaks, zero robotic failure reasons, and zero stack-like leaks. Prose summaries are not release proof. Legacy proof gaps may stay visible only when their listed planes include `gap_capsule_valid` counts matching `proof_gap`, matching `gap_ref`, `gap_backing complete`, and a matching `Legacy gap backing` detail row with complete backing, no latest gap, `release_blocking no`, a matching marked count, and a repair command.

Canary release validation also treats non-execution memory and voice planes as a proof contract. When `memory_movement_index`, `voice_surface_view`, or `voice_runtime_state` rows appear in the fresh audit transcript, every sampled row must be explicitly classified as `proof_n/a` with `proof_gap 0` and `gap_backing n/a`; otherwise the packet is stale or ambiguous and cannot support a release claim. Release checks also reject a stale source snapshot when source, test, ops, script, or docs files are newer than the packet's runtime evidence timestamp.
...
Final-answer proof is now a source-boundary contract. Delivered Telegram command acknowledgements must carry request, trace, and proof coverage when first written; suppressed Builder replies that do not have executable turn context must be marked as explicit non-execution proof instead of leaving silent trace/proof holes. Historical rows can be backfilled with `npm run control:proof:repair:final-answer`, but new rows should not depend on repair for release evidence.

The audit header now includes `Actionable status` and `Gap posture` to prevent `Status: gaps found` from being misread as a current blocker. Treat `Actionable status: clean` plus `Gap posture: backed legacy gaps only; no blocking or latest proof gaps` as the readable summary of the counters, not a replacement for them; release evidence still depends on `Blocking status: clean`, zero latest proof gaps, and complete legacy-gap backing.

The audit JSON result also carries `actionableStatus`, `gapPosture`, and `freshStrictOk` with the exact pass/fail value used by `--fresh-strict`. Downstream gates should read those fields directly instead of re-inferring the posture from `ok`, `blockingOk`, and legacy-gap counters when they need a compact machine-readable summary. `ok=false` can still be correct for backed historical gaps; `actionableStatus=clean` plus `freshStrictOk=true` is the machine-readable release-blocking proof gate when those gaps are complete, non-latest, and non-blocking.

The audit JSON and markdown report also carry `legacyGapBackingDetails` / `Legacy gap backing`. Each historical proof-gap plane must name its backing status, latest-gap status, release-blocking impact, repair source, and dry-run repair command. Use that detail when explaining why visible legacy gaps are allowed: acceptable release evidence requires `releaseBlocking=false`, complete backing, zero latest gaps, and matching plane-level proof-gap counts. If backing is incomplete, proof panels must name `incomplete legacy gap backing` in `Blocking gap planes` instead of hiding it behind a generic audit failure.

Canary observation packets now store a sanitized `evidence.controlProofAuditSummary` beside the human audit transcript. It preserves generated time, status, actionable status, blocking status, `freshStrictOk`, gap posture, gap counts, and gap plane labels without copying raw evidence file paths or trace rows. If that summary is present and disagrees with the transcript, the packet is invalid. Runtime evidence notes also include sanitized legacy repair dry-run counts for the visible historical planes, so `changed_rows=0` / `capsules_added=0` can be checked without reading raw trace files. The generated JSON summary exposes the parsed counts as `legacyRepairDryRunDetails`, so automation can confirm `changedRows=0`, `capsulesAdded=0`, and `parseErrors=0` without scraping notes. If those dry-run rows are present but missing a visible legacy plane, report parse errors, or show nonzero changed rows or capsule additions, the packet evidence is invalid under `legacy_repair_dry_run`.
...
- `releaseCaveatDetails.duplicate_truths.owner_sets` lists the sanitized owner set for duplicate-truth classifications such as `local_runtime_test_artifact`. The normalized publish handoff family for those rows is `local_runtime_test_artifacts`; automation should use `releaseCaveatDetails.duplicate_truths.label` or `publishHandoffs.local_runtime_test_artifacts` when explaining the operator-facing publish blocker.
...
Use `--repair-stale-proof-panels` only for already-reviewed canary cases whose saved `/proof` panel text is missing the current audit readiness lines (`Audit actionable`, `Audit blocking`, `Audit fresh-strict`, or `Audit posture`), `Blocking gap planes`, or the derived `Evidence capsule gaps` row. The command refuses to repair unless the packet already contains clean embedded fresh-strict audit evidence, then updates only those derived proof-panel status rows from existing proof-panel/audit evidence. It is for stale evidence maintenance, not for bypassing a missing live reply, screenshot, proof join, side-effect capture, user confirmation, or live-only proof-panel content.

Use `--summary-frozen-at-collected` only when regenerating checked-in summary fixtures from an already-reviewed observation packet. It freezes `now` at `evidence.collectedAt` and keeps the 1h packet window so tests can compare deterministic summary artifacts. It is not a live release check and must not be used to bypass `source_snapshot`, runtime freshness, or `--refresh-runtime-evidence` before a release-readiness claim.
...
7. Show partial trace evidence honestly. The proof panel reports ref-only evidence as joined when a redacted proof ref is present, names the missing capsule separately under `Evidence capsule gaps`, and keeps trace-only rows under `Evidence trace-only`; trace-only rows stay under `Evidence trace-only` and must not be counted as `Evidence joined`.
