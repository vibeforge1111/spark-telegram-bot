...
- `summary.cases` now carries safe per-canary Harness metadata: expected route, expected authority, expected mutation class, verdict, and missing capture names. It intentionally omits raw prompts, observed replies, proof-panel text, screenshots, and user confirmations.
