...
const COLLABORATIVE_IDEA_PATTERNS = [
  ...
  /\b(?:i|we)\b.{0,140}\b(?:want|need|would\s+like|would\s+love)\s+to\s+(?:build|make|create|develop)\s+(?:something|anything)\b.{0,140}\b(?:do\s+not|don't|dont|not\s+sure|unsure|haven't\s+figured\s+out|have\s+not\s+figured\s+out)\b.{0,100}\b(?:what|which|how|exactly|yet|should\s+(?:be|look|feel)|should\s+take)\b/i,
  /\b(?:i|we)\s+(?:want|need|would\s+like|would\s+love)\s+to\s+(?:build|make|create|develop)\s+(?:(?:a|an|the|tiny|small|simple|little|quick|basic|lightweight|mini|local|new)\s+){0,5}(?:tool|app|project|product|site|page|dashboard|game|timer|planner|picker|tracker)\b.{0,160}\b(?:not\s+sure|unsure|do\s+not\s+know|don't\s+know|dont\s+know|haven't\s+figured\s+out|have\s+not\s+figured\s+out|no\s+idea)\b.{0,100}\b(?:what|which|how|kind|type|form|shape|direction|exactly|yet|should\s+(?:be|look|feel)|should\s+take)\b/i,
  /\b(?:i|we)\b.{0,140}\b(?:want|need|would\s+like|would\s+love)\s+to\s+(?:build|make|create|develop)\s+(?:(?:a|an|the|tiny|small|simple|little|quick|basic|lightweight|mini|local|new)\s+){0,5}(?:tool|app|project|product|site|page|dashboard|game|timer|planner|picker|tracker)\b.{0,160}\b(?:not\s+sure|unsure|do\s+not\s+know|don't\s+know|dont\s+know|haven't\s+figured\s+out|have\s+not\s+figured\s+out|no\s+idea)\b.{0,100}\b(?:what|which|how|kind|type|form|shape|direction|exactly|yet|should\s+(?:be|look|feel)|should\s+take)\b/i,
  /\b(?:do\s+not|don't|dont)\s+(?:really\s+)?know\s+(?:exactly\s+)?(?:what|which|how)\b.{0,100}\b(?:it|this|that|thing|project|app|tool)?\s*(?:should\s+be|to\s+(?:build|make|create)|yet)\b/i,
  /\b(?:not\s+sure|unsure|no\s+idea)\b.{0,120}\b(?:what|which|how|kind|type|form|shape|direction|exactly|yet|should\s+(?:be|look|feel)|should\s+take|to\s+(?:build|make|create))\b/i,
...
export function isNoExecutionBoundary(text: string): boolean {
  ...
  return [
    ...
    /^(?:pause|cancel|stop)(?:[.!]+|\s*$)/,
    /\b(?:do not|don't|dont|please don't|please dont|no need to)\s+(?:start|run|launch|execute|open\s+(?:a\s+)?pr|kick\s+off)\b/,
