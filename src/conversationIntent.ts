...
const COLLABORATIVE_IDEA_PATTERNS = [
  ...
  /\b(?:i|we)\s+(?:want|need|would\s+like|would\s+love)\s+to\s+(?:build|make|create|develop)\s+(?:something|anything)\b.{0,140}\b(?:do\s+not|don't|dont|not\s+sure|unsure)\b.{0,80}\b(?:what|which|how|exactly|yet|should\s+be)\b/i,
  /\b(?:i|we)\s+(?:want|need|would\s+like|would\s+love)\s+to\s+(?:build|make|create|develop)\s+(?:(?:a|an|the|tiny|small|simple|local|new)\s+){0,5}(?:tool|app|project|product|site|page|dashboard|game|timer|planner|picker|tracker)\b.{0,160}\b(?:not\s+sure|unsure|do\s+not\s+know|don't\s+know|dont\s+know|no\s+idea)\b.{0,100}\b(?:what|which|how|kind|type|form|shape|direction|exactly|yet|should\s+be)\b/i,
  ...
  /\b(?:not\s+sure|unsure|no\s+idea)\b.{0,100}\b(?:what|which|how|kind|type|form|shape|direction|exactly|yet|should\s+be|to\s+(?:build|make|create))\b/i,
