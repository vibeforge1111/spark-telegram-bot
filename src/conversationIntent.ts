...
export function parseContextualSpawnerBoardNaturalIntent(text: string, recentMessages: string[] = []): SpawnerBoardNaturalIntent | null {
  ...
  const blockerPronounFollowup =
    /\b(?:what|which)\b.*\b(?:blocked|blocker|stopped|broke|failed|went\s+wrong)\b.*\b(?:that|it|this|that\s+one|this\s+one|the\s+one|the\s+broken\s+one|the\s+failed\s+one)\b/.test(normalized) ||
    /\b(?:that|it|this|that\s+one|this\s+one|the\s+one|the\s+broken\s+one|the\s+failed\s+one)\b.*\b(?:blocked|stopped|broke|failed|went\s+wrong)\b/.test(normalized) ||
    /\b(?:why|how)\b.*\b(?:that|it|this|that\s+one|this\s+one|the\s+one|the\s+broken\s+one|the\s+failed\s+one)\b.*\b(?:fail|failed|break|broke|blocked|stop|stopped)\b/.test(normalized);
  if (blockerPronounFollowup) {
    return hasRecentFailureContext ? 'latest_failure' : null;
  }

  const providerPronounFollowup =
    /\b(?:who|what|which\s+(?:llm|model|provider|agent))\b.*\b(?:took|handled|ran|accepted)\b.*\b(?:that|it|this|that\s+one|this\s+one|the\s+one|the\s+broken\s+one|the\s+failed\s+one)\b/.test(normalized) ||
    /\b(?:that|it|this|that\s+one|this\s+one|the\s+one|the\s+broken\s+one|the\s+failed\s+one)\b.*\b(?:who|what|which\s+(?:llm|model|provider|agent))\b.*\b(?:took|handled|ran|accepted)\b/.test(normalized);
