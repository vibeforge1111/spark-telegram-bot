...
async function httpStatus(url: string, timeoutMs = 3000): Promise<HttpStatusResult> {
  try {
    ...
    return { ok: false, err: String(err.code || err.message).replace(/\b\d{1,3}(\.\d{1,3}){3}\b/g, "[redacted]").replace(/:\d{2,5}\b/g, ":[redacted]") };
  }
}
...
async function pingProvider(providerId: string): Promise<PingResult> {
  ...
  try {
    ...
    return { providerId, ok: false, error: String(err.response?.data?.error || err.message).replace(/\b\d{1,3}(\.\d{1,3}){3}\b/g, "[redacted]").replace(/:\d{2,5}\b/g, ":[redacted]").slice(0, 200) };
  }
}
