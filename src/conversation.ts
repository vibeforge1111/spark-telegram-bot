...
export class ConversationMemory {
  ...
  private userKey(chatId: number, user: TelegramUser): string {
  ...
  async remember(chatId: number, user: TelegramUser, message: string): Promise<Memory | null> {
    await this.pushBounded(this.recentByUser, this.userKey(chatId, user), `User: ${message}`, this.maxRecent);
    await this.updateRollingFrame(chatId, user, 'user', message);
    return null;
  }

  async rememberAssistantReply(chatId: number, user: TelegramUser, message: string): Promise<Memory | null> {
    await this.pushBounded(this.recentByUser, this.userKey(chatId, user), `Spark: ${message}`, this.maxRecent);
    await this.updateRollingFrame(chatId, user, 'assistant', message);
    return null;
  }

  private async updateRollingFrame(chatId: number, user: TelegramUser, role: 'user' | 'assistant', text: string): Promise<void> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
  ...
  async learnAboutUser(chatId: number, user: TelegramUser, insight: string): Promise<Memory | null> {
    await this.pushBounded(this.notesByUser, this.userKey(chatId, user), insight, this.maxNotes);
    return null;
  }

  async storeAgentDoctrinePreference(chatId: number, user: TelegramUser, preference: string): Promise<Memory | null> {
    await this.ensureLoaded();
  ...
  async getAgentDoctrinePreferences(chatId: number, user: TelegramUser): Promise<string[]> {
    await this.ensureLoaded();
    const notes = this.notesByUser.get(this.userKey(chatId, user)) || [];
  ...
  async recordInterruptedTask(
    ...
    this.interruptedByUser.set(this.userKey(chatId, user), {
      message,
      failure,
  ...
  async getPendingTaskRecovery(chatId: number, user: TelegramUser): Promise<PendingTaskRecovery | null> {
    await this.ensureLoaded();
    return this.interruptedByUser.get(this.userKey(chatId, user)) || null;
  }

  async storePreference(chatId: number, user: TelegramUser, preference: string): Promise<Memory | null> {
    await this.pushBounded(this.notesByUser, this.userKey(chatId, user), `Preference: ${preference}`, this.maxNotes);
    return null;
  }

  async recall(chatId: number, user: TelegramUser, query: string, limit: number = 5): Promise<Memory[]> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
  ...
  async recallRecent(_chatId: number, _user: TelegramUser, _limit: number = 5): Promise<Memory[]> {
    return [];
  }

  async getContext(chatId: number, user: TelegramUser, _currentMessage: string): Promise<string> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
  ...
  async getRecentMessages(chatId: number, user: TelegramUser, limit: number = 6): Promise<string[]> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
    const recent = this.recentByUser.get(key) || [];
    return recent
  ...
  async getRecentTurns(chatId: number, user: TelegramUser, limit: number = 16): Promise<RecentConversationTurn[]> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
    const recent = this.recentByUser.get(key) || [];
    return recent
  ...
  async resolveRecentOptionReference(chatId: number, user: TelegramUser, text: string): Promise<ResolvedOptionReference | null> {
    ...
    await this.ensureLoaded();
    const recent = this.recentByUser.get(this.userKey(chatId, user)) || [];
  ...
  async getConversationFrame(chatId: number, user: TelegramUser, currentMessage: string): Promise<ConversationFrame> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
    ...
    const recentTurns = await this.getRecentTurns(chatId, user, 24);
  ...
  async getConversationFrameDiagnostics(chatId: number, user: TelegramUser): Promise<string> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
    return renderConversationFrameDiagnostics(this.frameStateByUser.get(key) || emptyRollingConversationFrameState());
  }

  async getMemoryCount(chatId: number, user: TelegramUser): Promise<number> {
    await this.ensureLoaded();
    const key = this.userKey(chatId, user);
