...
export async function runBuilderSelfImprovementPlan(
  ...
  } catch (error) {\n    throw new Error(`Builder self-improvement plan returned invalid JSON: ${error instanceof Error ? error.message : String(error)}. Output: ${trimmedStdout.slice(0, 300)}`);
  }
  return {
    payload,
