...
export const INTENT_PROPOSER_TAXONOMY: RouteDescriptor[] = [
  ...
  { route: 'recursive.proposal', useWhen: 'the user gives a fresh imperative to propose or prepare a recursive network proposal/packet, or to propose an improvement to a Spark subsystem for recursive/governed review', doNotUseWhen: 'they ask about, negate, quote, or discuss proposing one without asking Spark to prepare the proposal now', mutating: true },
  ...
  { route: 'memory.write', useWhen: 'the user explicitly asks to remember/save a fact for the future with a save imperative ("remember that...", "save this", "note that...", "/remember")', doNotUseWhen: 'they ask what is remembered, negate saving, or merely STATE a plan, opinion, or doctrine without a save imperative (e.g. "my plan is...", "agents should...", "memory update: <a plan>") - narrating a plan is discussion (plain_chat), not a memory write', mutating: true },
  { route: 'memory.delete', useWhen: 'the user explicitly asks to forget, stop remembering, or delete a stored memory/preference/fact, including natural phrasing like "forget that I prefer X", "stop remembering my X", or "delete the saved memory about X"', doNotUseWhen: 'they ask about forgetting, negate forgetting ("do not forget X", "don\'t forget to build X"), quote/report someone saying to forget, or say memory/data claims an unrelated action like "your memory says to delete the schedule"', mutating: true },
  ...
  { route: 'browser.navigate', useWhen: 'the user gives a fresh imperative to use the governed browser/browser-use/computer-use lane to open, visit, inspect, or read a specific public URL', doNotUseWhen: 'they ask whether browser use is available, discuss browser policy, quote a browser command, or say not to use/open the browser', mutating: false },
  { route: 'voice.command', useWhen: 'the user asks Spark to check live voice readiness, voice status, voice replies, transcription readiness, or to run a voice status/probe/diagnose check right now', doNotUseWhen: 'they quote/report/translate voice-status wording, mention voice in docs/examples, brainstorm voice UX/design, or ask to install/configure/enable/speak through voice', mutating: false },
  ...
  { route: 'spark.read_only_state', useWhen: 'the user asks a question about current live Spark system state, status, provider/runtime/access diagnostics, release blockers, or capability', doNotUseWhen: 'they ask about the user\'s project/task context, saved preferences, remembered facts, a project plan, blocker, constraint, stack, status, or what was discussed earlier - those are memory.recall or plain_chat, not Spark system state', mutating: false },
  ...
  { route: 'memory.recall', useWhen: 'the user explicitly asks what Spark remembers, what is saved, what was discussed or decided earlier, what the user previously stated, or which durable/recent memory facts exist', doNotUseWhen: 'they ask you to save (memory.write) or forget (memory.delete) something, ask about live Spark system/provider/diagnostic state (spark.read_only_state), or ask an open-ended planning/advice question such as what to focus on, work on, or do next - those are conversation.ideation unless the user explicitly asks what was remembered or discussed', mutating: false },
  { route: 'access.status', useWhen: 'the user asks what their current access level or operator status is', doNotUseWhen: 'they ask to CHANGE access or enable operator (that is access.change)', mutating: false },
  { route: 'conversation.ideation', useWhen: 'the user wants to think, plan, choose a next step, decide what to focus on, shape a project, or continue current conversational context without starting a tool action', doNotUseWhen: 'they explicitly ask what was remembered/saved/discussed earlier (memory.recall), ask for live Spark runtime/provider/access status (spark.read_only_state), or give a fresh imperative matching an action route', mutating: false },
