...
async function sendBuilderVoiceMedia(
  ...
  console.log(
    `[BridgeVoice] delivering media filename=${String(voiceMedia.filename).replace(/[ -]/g, '_')} mime=${voiceMedia.mimeType} voiceCompatible=${voiceMedia.voiceCompatible} bytes=${audioBuffer.length} captionChars=${caption?.length || 0} spokenChars=${(voiceMedia.spokenText || '').length}`
  );
