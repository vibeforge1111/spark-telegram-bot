...
export function scheduleSparkRestartAfterAccessChange(delayMs = 2_000): void {
  const script = [
    ...
    "const sparkBin = process.platform === 'win32' ? 'spark.cmd' : 'spark';",
    "const sparkPaths = [",
    "  path.join(os.homedir(), '.spark', 'bin', sparkBin),",
    "  sparkBin,",
    "];",
    "const spark = sparkPaths.find(p => { try { require('fs').accessSync(p, require('fs').constants.X_OK); return true; } catch { return false; } }) || sparkBin;",
